...
Arguments and return value are standard."
  ...
  (print-unreadable-object (q stream :type nil :identity t)
    (format stream
	    "QUEUE~@[ ~A~] [size ~D]~:[~;:~]"
	    (queue-name q)
	    (queue-size q)
	    (empty-p q))
    (when (not (empty-p q))
      (format stream " with first ~S last ~S:" (queue-first q) (queue-last q)))
    ))
...
(defun dequeue-items-if (predicate queue)
  ...
  (setf (queue-front queue) (delete-if predicate (queue-front queue))
	(queue-rear queue) (last (queue-front queue))
